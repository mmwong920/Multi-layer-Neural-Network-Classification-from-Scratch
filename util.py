...
def normalize_data(inp):
    ...
    """
    u = np.mean(inp, axis=0)
    sd = np.std(inp, axis=0)
    return ((inp - u) / sd)
...
def one_hot_encoding(labels, num_classes=10):
    ...
    """
    return np.eye(num_classes)[np.reshape(labels,(-1,)), :]
...
def calculateCorrect(y,t):  #Feel free to use this function to return accuracy instead of number of correct predictions
    ...
    """
    return np.sum(np.argmax(y,axis = 1) == onehot_decode(t))/np.shape(t)[0]
...
def createTrainValSplit(x_train,y_train):
    ...
    shuffle_y = y_train[p]
    shuffle_x = x_train[p]
    train_num = int(len(shuffle_x)*.8)
    # x_train, y_train, x_val, y_val
    return (shuffle_x[:train_num], shuffle_y[:train_num], shuffle_x[train_num:], shuffle_y[train_num:])
